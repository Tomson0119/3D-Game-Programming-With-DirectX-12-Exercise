...
Waves::Waves(int m, int n, float dx, float dt, float speed, float damping)
{
	...
	mPrevSolution.resize(size);
	mCurrSolution.resize(size);
	mNormals.resize(size);
	mTangentX.resize(size);

	// Generate grid vertices in system memory.

	float hWidth = (n - 1) * dx * 0.5f;
	float hDepth = (m - 1) * dx * 0.5f;
...
{
	return (mNumCols - 1) * mSpatialStep;
}

float Waves::Depth() const
{
	return (mNumRows - 1) * mSpatialStep;
}
...
void Waves::Update(float dt)
{
	...
	if (t >= mTimerStep)
	{
		...
		concurrency::parallel_for(1, mNumRows - 1, [this](size_t i)
			{
	...
	// this data needs to become the current solution and old
...
void Waves::Disturb(int i, int j, float magnitude)
{
	...
	mCurrSolution[(size_t)i * mNumCols + j].y       += magnitude;
	mCurrSolution[(size_t)i * mNumCols + j + 1].y   += halfMag;
	mCurrSolution[(size_t)i * mNumCols + j - 1].y   += halfMag;
	mCurrSolution[((size_t)i + 1) * mNumCols + j].y += halfMag;
	mCurrSolution[((size_t)i - 1) * mNumCols + j].y += halfMag;
}
